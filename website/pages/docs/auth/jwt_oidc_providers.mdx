...
- `gsuite_custom_schemas` `(string: <optional>` - Comma-separated list of G Suite [custom schemas](https://developers.google.com/admin-sdk/directory/v1/guides/manage-schemas).
Values set for G Suite users using custom schema fields will be fetched and made available as claims that can be used with [claim_mappings](/api/auth/jwt#claim_mappings).
- `gsuite_recurse_max_depth` `(int: <optional>` - Group membership recursion max depth. Default is 0, which means don't recurse.
...
vault write auth/oidc/config -<<EOF
{
    ...
    "provider_config": {
        ...
        "gsuite_custom_schemas": "Education,Preferences",
        "gsuite_recurse_max_depth": 5
    }
}
EOF
```
